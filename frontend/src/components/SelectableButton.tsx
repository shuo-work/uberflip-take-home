...
const selectableButton = ({isSelected, text, handleClick}: Props) => {
    const selectedButton = (
        <button className="inline-flex items-center px-4 sm:px-8 py-2 bg-white rounded-full shadow">
            ...
            <span className="ml-3 sm:ml-5 sm:text-lg font-medium">{text}</span>
        </button>
    );

    const unselectedButton = (
        <button onClick={handleClick} className="inline-block px-4 sm:px-10 sm:text-lg font-medium text-gray-500 hover:text-gray-600">
            {text}
        </button>
    );
    return (
